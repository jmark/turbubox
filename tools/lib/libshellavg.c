...
)
{
    ...
    const double I_max = .5*Nx + cxofs;
    const double J_max = .5*Ny + cyofs;
    const double K_max = .5*Nz + czofs;
    ...
    for (int k = 0; k < Nz; k++)
    {
        const double I = i - Nx*0.5 + cxofs;
        const double J = j - Ny*0.5 + cyofs;
        const double K = k - Nz*0.5 + czofs;
