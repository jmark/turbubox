...
def probe_couchdg(h5file):
    if 'meta_txt' in h5file.keys():
        if 'couchdg' in (x.decode().lower() for x in h5file.get('meta_txt').value):
            return couchdg.Ribbon
    return None
probes.append(probe_couchdg)
