...
for fp,lg in zip(fps,lgs):
    ...
    lg += ': P(rho^1/3*v)/k^%3.1f' % corr
    plt.loglog(xs,ys, '--', lw=3, label=lg)

plt.title("Turbulent Box (Mach = %d): Density-weighted Velocity Power Spectrum" % mach)
plt.xlabel('grid number: k')
plt.ylabel('density-weighted velocity spectrum: P(rho^1/3 * v)')
